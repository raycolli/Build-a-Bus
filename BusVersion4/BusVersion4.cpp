...
const char* progname = "CollierRachelFinalBus";
...
static void colorcube() {
    ...
    int numParticles = 100; // Adjust the number of particles
    float particleSize = 0.05; // Adjust the size of particles
    
    
    // Draw particles around back of bus centered
        for (int i = 0; i < numParticles; ++i) {
            // Random position
            float xPos = 2.0 * ((float)rand() / RAND_MAX) - 1.0;
            float yPos = 2.0 * ((float)rand() / RAND_MAX) - 1.0;
            float zPos = 2.0 * ((float)rand() / RAND_MAX) - 1.0;

            // Random distance
            float distance = 2.0 * ((float)rand() / RAND_MAX) + 1.0;
            ...
            // Draw the particle
            glPushMatrix();
            glTranslatef(xPos * distance, yPos * distance, zPos * distance);
            glutSolidCube(particleSize);
            glPopMatrix();
        }
...
/* void collision(int n)
 //tests for collisions and reflect particles if necessary
{
int i;
for (i=0; i<3; i++)
{
